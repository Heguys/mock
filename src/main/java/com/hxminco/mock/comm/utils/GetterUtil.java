...
public class GetterUtil {
	...
	public static Date get(Object value, DateFormat dateFormat,
			Date defaultValue) {
	...
	public static String get(String value, String defaultValue) {
		...
		if (value.indexOf(CharPool.RETURN) != -1) {
			value = StringUtil.replace(value, StringPool.RETURN_NEW_LINE,
					StringPool.NEW_LINE);
		}
	...
	private static String _trim(String value) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (Character.isDigit(c)
					|| ((c == CharPool.DASH) && ((i == 0)
							|| (value.charAt(i - 1) == CharPool.UPPER_CASE_E) || (value
							.charAt(i - 1) == CharPool.LOWER_CASE_E)))
