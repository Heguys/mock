...
public class StringUtil {
	...
	public static StringBuilder replaceToStringBuilder(String s, String begin,
	 ...
	 * 
	 * @param s
	 *            the string from which to strip all occurrences the character
	 * @param remove
	 ...
	 * 
	 * @param c
	 *            the character whose trim-ability is to be determined
	 * @param exceptions
	 *            the whitespace characters to exclude from trimming
	...
	/**
	 * upperToLowerAndPrependUnderline
	 * 
	 * @param content
	 * @return
	 */
	public static final String upperToLowerAndPrependUnderline(String content) {
		Pattern pattern = Pattern.compile(PATTERN_WORD_START_WITH_UPPER);
		while (Validator.isNotNull(content)) {
			Matcher matcher = pattern.matcher(content);
			if (matcher.find()) {
				String word = matcher.group().trim();
				String newWord = StringPool.UNDERLINE
						+ word.substring(0, 1).toLowerCase();
				if (word.length() > 1) {
					newWord += word.substring(1);
				}
				content = StringUtil.replace(content, word, newWord);
			} else {
				break;
			}
		}
		if (Validator.isNotNull(content)) {
			while (Validator.isNotNull(content)
					&& content.startsWith(StringPool.UNDERLINE)) {
	 ...
	 * @return
	 */
	public static final String replaceUnderlineAndFirstToUpper(String content) {
		Pattern pattern = Pattern
				.compile(PATTERN_WORD_START_WITH_UNDERLINE_lower);
		while (Validator.isNotNull(content)) {
			Matcher matcher = pattern.matcher(content);
			if (matcher.find()) {
				String word = matcher.group().trim();
				String newWord = word.substring(1, 2).toUpperCase();
				if (word.length() > 2) {
		...
		if (Validator.isNotNull(content)) {
			...
			if (firstChar.equals(firstChar.toUpperCase())) {
				content = firstChar.toLowerCase() + content.substring(1);
			}
		} else if (content == null) {
			content = StringPool.BLANK;
		}
	...
	private static final String PATTERN_WORD_START_WITH_UPPER = "[A-Z][a-z0-9]*";
	private static final String PATTERN_WORD_START_WITH_UNDERLINE_lower = "_[a-z][a-z0-9]*";
