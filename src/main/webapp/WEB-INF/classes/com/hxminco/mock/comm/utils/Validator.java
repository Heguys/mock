...
public class Validator {
	...
	public static boolean isDomain(String domainName) {
		...
		String[] domainNameArray = StringUtil
				.split(domainName, CharPool.PERIOD);
	 ...
	 * 
	 * @param month
	 *            the month (0-based, meaning 0 for January)
	 * @param day
		...
		for (int i = 0; i < number.length(); i++) {
			...
			if (((i + 1) % 2) == 0) {
				...
				if (x >= 10) {
					...
					x = GetterUtil.getInteger(s.substring(0, 1))
	...
	public static boolean isNull(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			if (c == CharPool.SPACE) {
				continue;
			} else if (c == CharPool.SPACE_UNICODE) {
				continue;
			} else if (counter > 3) {
				return false;
			}
